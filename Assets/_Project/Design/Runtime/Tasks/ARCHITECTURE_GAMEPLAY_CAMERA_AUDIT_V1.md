...
- `PROJECT_STATUS.md` still said the V23R19K Unity rerun was required;
...
- `ARCHITECTURE.md` when ownership changes;
- `TECHNICAL_DECISIONS.md` for durable choices;
- `QA_CHECKLIST.md` and automated QA when verification rules change;
- `PERFORMANCE_GUIDELINES.md` when budgets or profiling policy change;
- `DOCUMENTATION_INDEX.md` when maintained-document responsibility changes.
...
8. update `PROJECT_STATUS.md` in the same change;
...
- Resolved V23R19M/V23R19N bug rows are removed from the current open-bug table after their verified truth is summarized here and in `PROJECT_STATUS.md`.
