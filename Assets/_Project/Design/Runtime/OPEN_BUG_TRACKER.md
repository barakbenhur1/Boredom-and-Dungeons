...
| `BUG-V23R19O-001` | Target outline includes non-damageable enemy ring | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | The outline previously cloned every Mesh/Skinned renderer under the `BDHealth` root, including a broad presentation-only circle. V23R19O requires intersection with the real enabled non-trigger damageable collider envelope and excludes/fades auxiliary rings. |
...
- Those verified rows were removed from the current open table; Git history and `PROJECT_STATUS.md` retain the completed record.
