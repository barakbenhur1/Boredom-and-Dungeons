...
2. Relevant task and domain documents
   - a large or multi-session task receives an active task record under:
     `Assets/_Project/Design/Runtime/Tasks/`;
...
A task record never replaces `PROJECT_STATUS.md`, and `PROJECT_STATUS.md` never replaces the detailed task/domain record.
...
Update the task record, `PROJECT_STATUS.md`, and every relevant canonical document in the same change whenever:
...
4. Update `DOCUMENTATION_INDEX.md` in the same commit whenever a maintained document is added, removed, renamed, superseded, or changes responsibility.
...
2. update `PROJECT_STATUS.md` with the verified result and next resume point;
3. update or clear related entries in `OPEN_BUG_TRACKER.md`;
4. update `DOCUMENTATION_INDEX.md` when maintained-document ownership changes;
...
`Assets/_Project/Design/Runtime/Tasks/ARCHITECTURE_GAMEPLAY_CAMERA_AUDIT_V1.md`
