...
namespace BoredomAndDungeons
{
    public sealed partial class BDModernHandheld3DPresenter
    {
        ...
        private const float ChildApproachSceneFadeEndsAtSeconds = 0.55f;
        private const float ChildApproachWalkEndsAtSeconds = 3.45f;
        private const float ChildApproachClimbStartsAtSeconds = 3.78f;
        private const float ChildApproachSeatReachSeconds = 5.10f;
        private const float ChildApproachCameraSettleSeconds = 6.02f;
        private const float ChildApproachPowerOnStartsAtSeconds = 6.18f;
        private const float ChildApproachPowerOnEndsAtSeconds = 7.20f;
        ...
        private void ApplyChildApproachCameraPose(float progress)
        {
            ...
            if (seconds <= ChildApproachWalkEndsAtSeconds)
            {
                ...
                // Seven compact child steps begin only after the black scene
                // fade has revealed the room. The camera starts farther behind
                // and farther left, then advances at a slightly brisker but
                // grounded pace without becoming a floating or shaky camera.
                float stepEnvelope = Mathf.Sin(Mathf.PI * raw);
                float stepPhase = raw * Mathf.PI * 14f;
