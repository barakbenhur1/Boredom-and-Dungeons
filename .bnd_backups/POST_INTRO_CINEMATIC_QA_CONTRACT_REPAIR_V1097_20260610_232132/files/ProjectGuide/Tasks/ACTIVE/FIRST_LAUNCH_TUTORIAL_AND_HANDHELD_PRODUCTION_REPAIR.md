...
## V10.9.6 active blocking repair and exact resume point

The current task remains the post-intro cinematic. V10.9.6 implements the three latest user corrections in the existing presenter/camera/environment owners:

- prime the establishing camera before BBH fade exposes the scene;
- keep the approved portrait handheld flat and stationary on the tabletop;
- apply a constant realistic device/table scale and fit shadows/lights to it;
- retain one camera, one continuous 4.40-second shot and the same final ordinary-menu camera;
- extend focused QA without changing tutorial mechanics, menu semantics, device design, complete table geometry or agent-system files.

**Implementation state:** Runtime, focused QA and maintained documentation are supplied by the V10.9.6 package; Unity verification and user visual acceptance remain open.

**Exact resume point:** apply V10.9.6, compile, run `TEST EVERYTHING 0/0/0`, review the BBH handoff frame-by-frame and complete the documented scale/contact/24–60 FPS matrix. Do not advance to later work until accepted. The next task afterward remains the queued Atari/early-Nintendo-inspired tutorial visual redesign.
