...
## 2026-06-10 — Post-intro cinematic physical repair V10.9.6

**Classification:** `CURRENT / EARLIER-BLOCKING USER VISUAL REPAIR`

**User evidence:** the latest Play Mode review rejected three visible defects: a brief glimpse of the final/ordinary menu pose before the establishing shot, an upright device that should lie on the tabletop, and an unrealistic device-to-table size relationship.

**Implemented in V10.9.6:**

1. `PrimeIntroToMainMenuFirstFrame` applies the exact establishing camera position, look target, FOV, clip state, static scene pose and live screen while the BBH overlay still owns the frame. The cinematic clock starts only after that pose is applied, preventing a final/default camera frame from leaking through the BBH fade.
2. The existing portrait-form handheld now lies flat with `DeviceRestRotation = Quaternion.Euler(90f, 0f, 0f)` and rests at the real tabletop height. The device never rotates, slides or scales during the shot.
3. A constant `DeviceRealWorldScale = 0.16f` gives a `1.568 × 2.464` project-unit tabletop footprint against the existing `24 × 12.4` table. The short axis is `6.53%` of table width and the long axis is `19.87%` of table depth, equivalent to approximately a `9.8 × 15.4 cm` portable device on a `150 × 78 cm` table.
4. The ordinary Main Menu camera was re-authored for the flat device: it approaches from the front, remains high enough to read the screen, keeps the tabletop front edge visible and uses a restrained perspective lens. The opening spline remains high/far/left and ends exactly on this same ordinary menu state.
5. Device penumbra/core/contact masks and the three-light rig now target the smaller horizontal footprint. The complete table, floor, cyclorama and all object transforms remain static; only the product camera/lens/look state animates.
6. Focused handheld and first-launch QA now require first-frame priming, the flat rest pose, constant realistic scale, the new camera contract and scaled horizontal shadow masks, while explicitly rejecting the old upright pose, old camera and oversized shadow dimensions.

**Preservation:** V10.8.1 tutorial mechanics, menu behavior, device mesh/material/control design, complete V10.9 table construction, agent-system files and later queue are unchanged.

**QA truth:** package/static validation covers first install, idempotent install, protected-file equality, source token/forbidden-token checks, C# delimiter/preprocessor balance, rollback, terminal semantics and `git diff --check`. Unity compilation, rendered continuity, 24/30/60 FPS behavior and user visual acceptance remain unverified.

**Exact resume point:** apply V10.9.6, let Unity `6000.0.76f1` compile, run `Boredom And Dungeons -> TEST EVERYTHING` and require `0 blockers / 0 warnings / 0 info`. Then inspect the BBH handoff frame-by-frame, verify the flat grounded device and measured proportions, review the full shot at 24-equivalent/30/60 FPS, and confirm no replay on internal menu returns. Only after user acceptance continue to `Tasks/QUEUED/FIRST_LAUNCH_TUTORIAL_RETRO_VISUAL_REDESIGN.md`.
