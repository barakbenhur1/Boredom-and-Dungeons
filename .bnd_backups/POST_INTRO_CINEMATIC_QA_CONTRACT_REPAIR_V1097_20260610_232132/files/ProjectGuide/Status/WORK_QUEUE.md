...
## Current immediate gate — V10.9.6 physical repair

1. Apply the V10.9.6 backup-aware package without resetting local work or changing the Codex-agent system.
2. Let Unity `6000.0.76f1` compile; resolve every project-generated compiler error or warning.
3. Run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
4. Scrub the BBH fade and first cinematic frames one frame at a time. Confirm the first exposed frame is already the high/far/left establishing pose and no final/default pose flashes first.
5. Confirm the device lies flat on the tabletop at all times, with no transform/scale change and no gap or penetration.
6. Confirm measured project ratios: `1.568 / 24 = 6.53%` across table width and `2.464 / 12.4 = 19.87%` along table depth.
7. Review 0.0s, 1.3s, 2.2s, 3.3s and final frame; verify complete table/floor at start, continuous camera motion, readable screen, front edge/thickness at end and exact stable handoff.
8. Repeat at 24-equivalent capture, 30 FPS and 60 FPS, including Settings/Progression/Credits returns with no replay.
9. Recheck V10.8.1 mounted-shot and tutorial regression items.
10. Commit only after user acceptance, then continue to the queued retro tutorial visual redesign.
