...
## Post-intro cinematic V10.9.6 — repair implemented, Unity verification pending

| ID | Area | Status | Current truth / acceptance condition |
|---|---|---|---|
| `CINEMATIC-V1096-001` | Pre-roll frame continuity | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | The exact establishing pose is primed while BBH still covers the frame and the movement clock starts only afterward. Acceptance requires frame-by-frame proof that no ordinary/final/default camera pose, wrong FOV or wrong screen frame appears before frame 0. |
| `CINEMATIC-V1096-002` | Handheld physical rest pose | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | The portrait device uses a fixed 90-degree X rest rotation and lies flat at the tabletop contact height. It must remain stationary and grounded for the shot and all ordinary menu pages; screen readability comes only from camera placement. |
| `CINEMATIC-V1096-003` | Table/device physical scale | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | Static scale `0.16` produces a `1.568 × 2.464` footprint on the `24 × 12.4` table: `6.53%` of table width and `19.87%` of table depth, matching a portable handheld on full-size furniture. Acceptance requires no scale change and believable wide/final frames. |
| `CINEMATIC-V1096-004` | Flat-pose shadow and lighting fit | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | Device contact masks and lighting targets were resized/repositioned for the horizontal footprint. Acceptance requires a tight contact shadow, readable soft left penumbra, no old giant shadow and no floating/penetrating device. |

The V10.9.5 open documentation rows are superseded by these implementation rows. Close them only after clean Unity compilation, `TEST EVERYTHING 0/0/0`, focused frame/ratio checks and user visual acceptance.
