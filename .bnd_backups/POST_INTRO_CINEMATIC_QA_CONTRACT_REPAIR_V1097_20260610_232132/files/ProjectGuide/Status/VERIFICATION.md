...
## V10.9.6 verification truth — 2026-06-10

### Recorded user evidence

The user reported a pre-roll glimpse of the final pose and supplied a Play Mode screenshot showing an upright, oversized handheld on the new full table. This is accepted defect evidence.

### Package/static evidence

- exact V10.9.1 transition/environment baselines accepted, with idempotent V10.9.6 state also accepted;
- presenter rest-pose/scale anchors patched additively;
- exact replacement transition/environment files validated by SHA-256;
- focused handheld and tutorial QA require priming, flat pose, measured scale and resized shadows and reject superseded values;
- non-owned repository files compared byte-for-byte before/after installation;
- backup and byte-for-byte rollback verified;
- interactive semantic colors and ANSI-free compatibility modes verified;
- full-line merge-conflict detection and `git diff --check` verified;
- C# brace, parenthesis, bracket and preprocessor balance verified for all touched C# files.

### Not yet verified

- Unity compilation after V10.9.6;
- `TEST EVERYTHING` after V10.9.6;
- actual first-frame continuity through BBH fade;
- rendered contact, lighting and screen readability;
- 24/30/60 FPS motion equivalence;
- final user visual acceptance.

The measured static ratio is `DeviceWidth × 0.16 / 24 = 6.53%` of table width and `DeviceHeight × 0.16 / 12.4 = 19.87%` of table depth. Runtime transforms/scales must remain constant throughout playback.
