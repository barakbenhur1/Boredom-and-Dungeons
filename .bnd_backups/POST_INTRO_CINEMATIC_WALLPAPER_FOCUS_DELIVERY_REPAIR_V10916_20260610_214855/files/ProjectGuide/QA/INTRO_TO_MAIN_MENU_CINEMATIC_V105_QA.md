...
## V10.9.13 aggregate-source QA contract

`BDFirstLaunchTutorialQA` must:

1. list `BDModernHandheld3DPresenter.CinematicDepthOfField.cs` in `RequiredFiles`;
2. require the `BDCinematicDepthOfField` runtime token;
3. construct `cinematicDepthOfFieldPath`;
4. concatenate `ReadIfPresent(cinematicDepthOfFieldPath)` into `runtime` before checking tokens.

The repair must not change the V10.9.12 Runtime, shader, camera, environment or placement files. Acceptance requires Unity compile and `TEST EVERYTHING 0/0/0`.
