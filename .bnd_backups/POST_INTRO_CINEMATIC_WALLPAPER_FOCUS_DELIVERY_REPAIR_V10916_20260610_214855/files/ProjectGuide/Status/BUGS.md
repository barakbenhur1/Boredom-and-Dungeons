...
## Post-intro cinematic V10.9.13 — DOF QA aggregation blocker

| ID | Area | Status | Acceptance condition |
|---|---|---|---|
| `CINEMATIC-V10913-001` | First-launch aggregate QA | `FIX IMPLEMENTED / UNITY VERIFICATION REQUIRED` | Aggregate scanner reads `BDModernHandheld3DPresenter.CinematicDepthOfField.cs` before checking `BDCinematicDepthOfField`. |
| `CINEMATIC-V10913-002` | Regression prevention | `IMPLEMENTED` | QA source must contain both the new path variable and `ReadIfPresent(cinematicDepthOfFieldPath)`. |
| `CINEMATIC-V10912-001` | Focused-room visual pass | `IMPLEMENTED / VISUAL VERIFICATION REQUIRED` | V10.9.12 camera, room, placement and DOF remain unchanged by this QA-only repair. |

Incoming evidence: Unity `6000.0.76f1`, generated UTC `2026-06-10T21:31:03.8584510Z`, automated `BLOCKED`, `1 blocker / 0 warnings / 0 info`.
