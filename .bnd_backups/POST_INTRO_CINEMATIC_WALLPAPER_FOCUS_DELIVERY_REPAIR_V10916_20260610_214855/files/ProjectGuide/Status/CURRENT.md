<!-- BND_POST_INTRO_CINEMATIC_DOF_QA_AGGREGATION_REPAIR_V10913:BEGIN -->
## 2026-06-11 — Post-intro cinematic DOF QA aggregation repair V10.9.13

**Classification:** `CURRENT / AUTOMATED-QA REPAIR`

**Repository baseline:** commit `38ac49268a9afa0d711bd3cb8c1668c49cdc079f` with V10.9.12 installed in the local working tree.

The first Unity run after V10.9.12 compiled and reached `TEST EVERYTHING`, but automation reported one blocker:

```text
FIRST_LAUNCH_TUTORIAL_CONTRACT_INVALID
Missing first-launch tutorial contract token: BDCinematicDepthOfField
```

The implementation file exists and the focused handheld validator scans it correctly. The first-launch aggregate validator added the file to `RequiredFiles` and added `BDCinematicDepthOfField` to `RequiredRuntimeTokens`, but it did not read the new file into its concatenated `runtime` source string. The result was a false missing-token blocker.

V10.9.13 adds `cinematicDepthOfFieldPath` and includes `ReadIfPresent(cinematicDepthOfFieldPath)` in the aggregate runtime scan. No Runtime, camera, room, shader, device-placement or tutorial behavior changes.

**Exact resume point:** apply V10.9.13, wait for Unity compilation, rerun `Boredom And Dungeons -> TEST EVERYTHING`, require `0 blockers / 0 warnings / 0 info`, then continue visual review of V10.9.12.
<!-- BND_POST_INTRO_CINEMATIC_DOF_QA_AGGREGATION_REPAIR_V10913:END -->
