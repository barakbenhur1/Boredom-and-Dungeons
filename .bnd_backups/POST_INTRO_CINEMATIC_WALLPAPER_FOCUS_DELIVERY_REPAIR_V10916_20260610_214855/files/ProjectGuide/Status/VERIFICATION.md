...
`BDFirstLaunchTutorialQA` required the new DOF file and token but omitted the file from its concatenated runtime scan.
...
```csharp
string cinematicDepthOfFieldPath = Path.Combine(...);
```

and:

```csharp
ReadIfPresent(cinematicDepthOfFieldPath) +
```

No production Runtime or shader file is changed.

### Required evidence

Fresh Unity compile, `TEST EVERYTHING 0/0/0`, then V10.9.12 visual acceptance.
