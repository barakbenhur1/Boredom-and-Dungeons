...
## V10.9.8 transform ownership and tabletop clearance

The physical scale remains `0.16`; the prior giant rendering was caused by a later frame update replacing the root scale with `Vector3.one`, not by the documented ratio. The only legal persistent device transform is:
...
The back-shell geometry reaches approximately local `z = +0.70`; after the 90-degree rotation and scale `0.16`, its lowest point is about `0.112` units below the root. Against the world tabletop surface at `-7.40`, root `-7.27` leaves roughly `0.018` units of physical clearance before the contact shadow. The device must never be moved or enlarged by generic menu-entry animation.
