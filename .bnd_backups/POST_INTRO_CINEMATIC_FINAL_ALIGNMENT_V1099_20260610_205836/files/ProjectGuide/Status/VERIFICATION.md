...
## V10.9.8 verification truth — 2026-06-10

### Recorded user evidence

Two Play Mode screenshots show the device occupying roughly half the table width, visibly intersecting the tabletop, and ending on a low grazing close-up that crops the hardware and distorts the screen. This is accepted failure evidence; V10.9.6 is not visually approved.

### Confirmed source causes

- `UpdateEntryAnimation` wrote `Vector3.Lerp(Vector3.one * 0.94f, Vector3.one, eased)` every visible frame, overriding `DeviceRestScale` and moving/rotating the flat device.
- `UpdateScreenResolution` wrote `Mathf.Lerp(49f, 36.4f, fit)` whenever the cinematic was inactive, overriding the intended final lens immediately after completion.
- the old rest root `-7.31f` did not include the maximum back-shell extent at scale `0.16`.

### V10.9.8 static/package evidence

The package requires and replaces the exact conflicting methods/tokens, updates both focused validators, synchronizes maintained documentation, verifies protected files byte-for-byte, supports idempotent reapplication, performs automatic rollback on post-write failure, checks full-line conflict markers, C# delimiter/preprocessor balance, terminal color compatibility and `git diff --check`.

### Still required

Unity compilation, fresh `TEST EVERYTHING`, rendered scale/contact/final-angle review at 24/30/60 FPS, and user visual acceptance.
