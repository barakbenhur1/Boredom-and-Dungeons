...
## 2026-06-10 — Post-intro cinematic visual repair V10.9.8

**Classification:** `CURRENT / EARLIER-BLOCKING USER VISUAL REPAIR`

The latest Play Mode screenshots reject V10.9.6 visually: the handheld still renders enormous relative to the table, penetrates the tabletop, and the final camera is too low and distorted.

**Confirmed root causes and implemented correction:**

1. `UpdateEntryAnimation` was still executed every visible frame and overwrote the documented `DeviceRestScale` with a lerp ending at `Vector3.one`. This made the device render at full mesh scale despite `DeviceRealWorldScale = 0.16f`; it also animated position/rotation and drove the flat device through the tabletop. V10.9.8 removes that competing transform animation and enforces the exact static rest position, rotation and scale every frame.
2. `UpdateScreenResolution` still overwrote the cinematic final lens with the retired `49f -> 36.4f` FOV immediately after the transition. V10.9.8 routes all ordinary-menu aspect changes through `ResolveRegularMainMenuFieldOfView`, so the final frame cannot jump to the old low/distorted composition.
3. The rest height is raised from `-7.31f` to `-7.27f`, accounting for the deepest back-shell geometry after the 90-degree rotation and leaving a small non-penetrating contact clearance above the `-7.40f` tabletop surface.
4. The final ordinary-menu camera is re-authored to a higher downward product angle at `(0, 1.50, -14.0)`, looking at `(0, -7.20, 0)`, with restrained `17f -> 15f` aspect-aware FOV. The spline approaches that exact pose without a corrective frame.
5. The physical device scale remains `0.16`: once the competing `Vector3.one` write is removed, the intended `1.568 × 2.464` footprint finally applies. The table, device and shadow roots remain static; only the camera/lens/look state animates.

**Preservation:** no tutorial mechanic, menu action, model geometry, material, texture, complete table construction, agent-system file or queued task is removed or reinterpreted.

**Exact resume point:** apply V10.9.8, compile in Unity `6000.0.76f1`, run `Boredom And Dungeons -> TEST EVERYTHING` and require `0/0/0`, then review the BBH handoff and final frame. Do not advance to the retro tutorial redesign until the device is visibly small, fully above the tabletop and the final screen angle is accepted by the user.
