...
## Current immediate gate — V10.9.8 post-intro visual repair

1. Apply V10.9.8 over the current V10.9.6/V10.9.7 local working tree without reset, stash or discarded work.
2. Compile in Unity `6000.0.76f1` and run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
3. Verify the device is small relative to the full table from the establishing frame onward; no frame may use scale `1`.
4. Inspect the full device underside/back shell against the tabletop. Require a thin contact clearance with tight shadow, no penetration and no visible floating.
5. Verify the final ordinary-menu camera is higher and more downward-facing, the complete screen is readable without grazing distortion, and no post-settle FOV change occurs.
6. Scrub the last BBH frames and first cinematic frames for no flicker/snap; inspect 0.0s, 1.3s, 2.2s, 3.3s and final.
7. Repeat at 24-equivalent, 30 FPS, 60 FPS and after Settings/Progression/Credits returns; the special shot must not replay and the static pose/scale must remain identical.
8. Recheck V10.8.1 mounted-shot/tutorial regression items.
9. Commit only after user acceptance. The next task remains `Tasks/QUEUED/FIRST_LAUNCH_TUTORIAL_RETRO_VISUAL_REDESIGN.md`.
