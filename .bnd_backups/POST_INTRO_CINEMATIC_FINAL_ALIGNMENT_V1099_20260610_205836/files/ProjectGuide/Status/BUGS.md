...
## Post-intro cinematic V10.9.8 — visual rejection repair implemented, Unity verification pending

| ID | Area | Status | Current truth / acceptance condition |
|---|---|---|---|
| `CINEMATIC-V1098-001` | Device scale ownership | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | `UpdateEntryAnimation` previously restored `Vector3.one`, defeating scale `0.16`. It now enforces `DeviceRestScale`; the device must read as a small handheld on full-size furniture in every frame and menu page. |
| `CINEMATIC-V1098-002` | Table penetration | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | The competing position/rotation animation is removed and the rest root is raised to `-7.27f`, clearing the deepest back shell above the `-7.40f` top. No mesh may intersect or float. |
| `CINEMATIC-V1098-003` | Final camera/lens | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | The final camera is higher and farther, uses a direct downward product angle and `17f -> 15f` FOV. `UpdateScreenResolution` may not restore the retired `49f -> 36.4f` lens. |
| `CINEMATIC-V1098-004` | First-frame continuity | `IMPLEMENTED / UNITY VERIFICATION REQUIRED` | V10.9.6 priming remains. Verify no final/default frame precedes the establishing shot and no FOV correction occurs after settle. |
| `DELIVERY-V1097-001` | QA contradiction | `IMPLEMENTED / UNITY RERUN REQUIRED` | The stale `0.28f` upright-position requirement remains removed. V10.9.8 updates the validators to protect the corrected transform/lens ownership. |

Close only after `TEST EVERYTHING 0/0/0`, focused Play Mode review and explicit user visual acceptance.
