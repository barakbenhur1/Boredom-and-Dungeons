...
## V10.9.8 active visual repair and exact resume point

The current task remains the post-intro cinematic. User Play Mode evidence rejected V10.9.6 because a generic presenter entry animation overrode scale/pose and a legacy resolution path overrode the final lens. V10.9.8 repairs those real owners, raises the device to a non-penetrating contact height, re-authors the final camera, updates both validators and synchronizes the maintained documents.

**Exact resume point:** apply V10.9.8, compile, run `TEST EVERYTHING 0/0/0`, then visually verify realistic scale, zero tabletop clipping, improved final angle, BBH continuity and 24/30/60 FPS stability. Do not advance until user acceptance; afterward resume the queued retro tutorial visual redesign.
