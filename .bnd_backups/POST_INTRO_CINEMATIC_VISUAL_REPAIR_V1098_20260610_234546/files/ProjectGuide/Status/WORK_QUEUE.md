...
## Current immediate gate — V10.9.7 QA contract repair

1. Apply V10.9.7 over the installed V10.9.6 working tree; do not reset, stash or discard local work.
2. Let Unity `6000.0.76f1` recompile.
3. Run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
4. If clean, continue the V10.9.6 visual gate: scrub the BBH handoff frame-by-frame, verify no pre-roll final-pose flash, confirm the device lies flat and grounded, and confirm the `0.16` scale/proportions remain constant.
5. Review 0.0s, 1.3s, 2.2s, 3.3s and final frame at 24-equivalent, 30 FPS and 60 FPS; verify no replay on internal menu returns.
6. Recheck the V10.8.1 tutorial regression items.
7. Commit only after user acceptance. The next task remains the queued Atari/early-Nintendo-inspired tutorial visual redesign.
