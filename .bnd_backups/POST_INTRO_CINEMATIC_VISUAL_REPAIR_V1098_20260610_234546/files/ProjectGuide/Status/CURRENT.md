...
## 2026-06-10 — Post-intro cinematic QA contract repair V10.9.7

**Classification:** `CURRENT / BLOCKING AUTOMATED-QA REPAIR INSIDE THE V10.9.6 CINEMATIC GATE`

**Recorded Unity evidence:** Unity `6000.0.76f1` generated `TEST EVERYTHING: BLOCKED` at `2026-06-10T20:13:45.0121340Z` with exactly one blocker, zero warnings and zero info. `HANDHELD_3D_PRESENTER_MISSING` reported the retired token `new Vector3(0f, 0.28f, 0f)`.

**Root cause:** V10.9.6 correctly replaced the old upright-device rest position with the flat tabletop pose, and focused tutorial QA already forbids the retired value. A separate legacy requirement remained in `BDModernHandheld3DQA`, so the automated gate simultaneously rejected the correct Runtime state and demanded the superseded state.

**Implemented in V10.9.7:** remove only that stale requirement from the handheld validator. Retain all V10.9.6 requirements for `DeviceRealWorldScale = 0.16f`, `DeviceRestScale`, `new Vector3(0f, -7.31f, 0f)`, `Quaternion.Euler(90f, 0f, 0f)`, first-frame priming, the new camera, resized shadows and the complete 3D environment. Runtime, scene, device, camera, table, tutorial mechanics and agent-system files are unchanged.

**QA truth:** the uploaded blocked result proves the validator defect but does not disprove the V10.9.6 Runtime implementation. V10.9.7 package/static checks validate the exact QA edit, document synchronization, protected-file byte equality, idempotency, rollback and `git diff --check`. A fresh Unity compile, `TEST EVERYTHING` and focused visual review remain required.

**Exact resume point:** apply V10.9.7, allow Unity to recompile and rerun `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`. Then continue the already-defined V10.9.6 frame-by-frame, flat-contact, physical-scale and 24/30/60 FPS visual matrix. Do not advance to the retro tutorial redesign until user acceptance.
