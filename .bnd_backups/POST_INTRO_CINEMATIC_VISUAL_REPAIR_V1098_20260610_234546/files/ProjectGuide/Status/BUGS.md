...
## Post-intro cinematic V10.9.7 — automated QA contradiction repaired, rerun pending

| ID | Area | Status | Current truth / acceptance condition |
|---|---|---|---|
| `DELIVERY-V1097-001` | Handheld QA rest-pose contract | `IMPLEMENTED / UNITY RERUN REQUIRED` | Unity reported one blocker because `BDModernHandheld3DQA` still required the retired upright-pose position `new Vector3(0f, 0.28f, 0f)`. V10.9.7 removes only that contradictory requirement and retains the complete flat-pose, realistic-scale, first-frame and environment contract. Acceptance requires a fresh `TEST EVERYTHING 0/0/0`. |
| `CINEMATIC-V1096-001` | Pre-roll frame continuity | `IMPLEMENTED / UNITY VISUAL VERIFICATION REQUIRED` | The establishing pose is primed while BBH still covers the frame. Verify no ordinary/final/default pose appears before frame 0. |
| `CINEMATIC-V1096-002` | Flat handheld rest pose | `IMPLEMENTED / UNITY VISUAL VERIFICATION REQUIRED` | The device remains at the fixed tabletop position, 90-degree X rotation and scale `0.16`; screen readability comes from the camera only. |
| `CINEMATIC-V1096-003` | Physical table/device scale | `IMPLEMENTED / UNITY VISUAL VERIFICATION REQUIRED` | The `1.568 × 2.464` footprint remains constant on the `24 × 12.4` table. Verify believable proportions and no scale animation. |
| `CINEMATIC-V1096-004` | Shadow/contact fit | `IMPLEMENTED / UNITY VISUAL VERIFICATION REQUIRED` | Resized contact/core/penumbra masks must ground the device without the retired giant shadow. |

The uploaded blocker is a validator defect, not visual acceptance. These Runtime rows remain open until automated and focused Play Mode gates both pass.
