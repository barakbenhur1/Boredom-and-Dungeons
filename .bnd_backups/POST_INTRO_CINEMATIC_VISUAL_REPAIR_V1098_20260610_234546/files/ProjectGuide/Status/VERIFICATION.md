...
## V10.9.7 verification truth — 2026-06-10

### Recorded Unity result before this repair

```text
Generated UTC: 2026-06-10T20:13:45.0121340Z
Unity: 6000.0.76f1
...
Missing required contract: new Vector3(0f, 0.28f, 0f)
```

The missing token is intentionally retired by V10.9.6. The focused tutorial validator already rejects it, while the handheld validator still required it. V10.9.7 removes that single contradictory requirement and preserves every current V10.9.6 assertion.

### V10.9.7 package/static evidence

- exact stale token line removed only from `BDModernHandheld3DQA.cs`;
- current flat-pose, scale, camera, first-frame and cinematic-environment QA tokens retained;
- Runtime presenter, transition, environment, tutorial QA, scene, AGENTS, Codex agents, skills and repository-rule documents remain byte-identical;
- first application and idempotent reapplication pass;
- unsupported pre-V10.9.6 state blocks before writing;
- forced post-write failure restores every owned file byte-for-byte and removes the failed backup;
- semantic terminal colors and ANSI-free compatibility modes pass;
- ZIP path safety, duplicate-member checks, C# delimiter/preprocessor balance and `git diff --check` pass.

### Still required

- fresh Unity compilation;
- `TEST EVERYTHING: 0 blockers / 0 warnings / 0 info`;
- full V10.9.6 focused visual matrix and user acceptance.
