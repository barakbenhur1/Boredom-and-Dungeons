...
## V10.9.7 active blocking QA repair and resume point

The current task remains the post-intro cinematic. The latest Unity run reached the automated gate but was blocked by one stale handheld-validator requirement for the retired V10.9.5 upright position. V10.9.7 removes only that contradiction.

**Preserved implementation:** V10.9.6 first-frame priming, flat tabletop pose, constant physical scale, camera path, resized shadows, complete table environment, menu semantics, tutorial mechanics and agent-system files remain unchanged.

**Exact resume point:** apply V10.9.7, recompile and rerun `TEST EVERYTHING 0/0/0`; then complete the documented V10.9.6 visual review. Do not start the retro tutorial redesign before this cinematic is accepted.
