...
## V10.9.6 physical table-scene implementation

The handheld keeps its portrait industrial design but is physically staged flat on the table.

### Project-unit dimensions

| Object | Width | Front-to-back length/depth | Thickness/height |
|---|---:|---:|---:|
| Unscaled handheld mesh | `9.8` | `15.4` | `0.92` |
| Handheld at fixed scale `0.16` | `1.568` | `2.464` | `0.1472` |
| Table | `24` | `12.4` | full top/apron/leg construction |

- device short axis / table width: `1.568 / 24 = 6.53%`;
- device long axis / table depth: `2.464 / 12.4 = 19.87%`;
- this corresponds approximately to a `9.8 × 15.4 × 0.92 cm` handheld on a `150 × 77.5 cm` table when `0.16` project units represent one centimetre in the device/table ratio family.

The fixed rest transform uses a 90-degree X rotation so the front glass faces upward and the top of the portrait device points away from the front camera. The root height places the back shell directly above the tabletop with a small non-penetrating contact allowance. No object scale or transform animation is legal; camera/lens movement owns composition.
