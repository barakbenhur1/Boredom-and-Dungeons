...
## V10.9.12 active implementation

The active blocking task remains the post-intro cinematic. Implement and verify the tighter direct final frame, slightly more forward device placement, lived-in room dressing and device-focused photographic depth of field. Preserve all accepted tutorial mechanics and the existing camera-only spline architecture. Completion requires Unity compile, `TEST EVERYTHING 0/0/0` and explicit user visual approval.
