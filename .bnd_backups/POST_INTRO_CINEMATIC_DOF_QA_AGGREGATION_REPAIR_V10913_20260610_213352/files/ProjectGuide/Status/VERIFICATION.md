...
## V10.9.12 verification contract

### Static/package evidence

The installer is built against Git commit `38ac49268a9afa0d711bd3cb8c1668c49cdc079f` and owns only the focused cinematic Runtime, shader, QA and maintained-document files. It verifies exact patch anchors, tracked-file stability outside the owned set, C# delimiter/preprocessor balance, absence of trailing method-call commas, shader structure, idempotent application, rollback and `git diff --check`.

### Unity evidence required

- Unity `6000.0.76f1` compiles without warnings/errors introduced by this package.
- `Boredom And Dungeons -> TEST EVERYTHING` returns `PASS`, `0 blockers`, `0 warnings`, `0 info`.
- The custom depth effect is active on the product camera and falls back to an unmodified blit if unsupported.
- Final framing remains direct and stable after the cinematic completes and after resizing the Game view.
- Visual acceptance is still user-owned and is not claimed by package tests.
