...
## Current task gate — V10.9.12 focused room polish

1. Apply V10.9.12 on baseline `38ac49268a9afa0d711bd3cb8c1668c49cdc079f`.
2. Let Unity compile all new Runtime and shader files.
3. Run `Boredom And Dungeons -> TEST EVERYTHING`; require `0/0/0`.
4. Review opening, 1.3s, 2.2s, 3.3s and final frame.
5. Verify:
   - richer room reads during the wide portion but remains subordinate;
   - depth of field keeps the handheld and tabletop contact sharp;
   - no blur obscures the live screen;
   - device is slightly nearer the front edge with safe wood margin;
   - final direct view is materially closer and fills most of the frame;
   - no clipping, jitter, camera snap, scale animation or replay on internal menu returns.
6. Keep the retro tutorial redesign queued until explicit cinematic acceptance.
