...
## Post-intro cinematic V10.9.12 — active visual findings

| ID | Area | Status | Acceptance condition |
|---|---|---|---|
| `CINEMATIC-V10912-001` | Final framing | `IMPLEMENTED / VISUAL VERIFICATION REQUIRED` | Handheld fills roughly 70%–80% of final-frame height on a wide Game view, with complete shell and controls visible and no perspective distortion. |
| `CINEMATIC-V10912-002` | Table placement | `IMPLEMENTED / VISUAL VERIFICATION REQUIRED` | Device rests at `z=-4.28`; its front edge remains about `1.69` world units behind the table edge, with no clipping or unstable overhang. |
| `CINEMATIC-V10912-003` | Room quality | `IMPLEMENTED / VISUAL VERIFICATION REQUIRED` | Opening/mid shot includes restrained room dressing rather than an empty placeholder cyclorama: rug, slatted wall, credenza, practical lamp, books, plant and framed accents. |
| `CINEMATIC-V10912-004` | Focus hierarchy | `IMPLEMENTED / VISUAL VERIFICATION REQUIRED` | Custom depth-of-field keeps the handheld and immediate tabletop sharp; foreground floor and distant room props soften naturally. No blur may reduce menu readability. |
| `CINEMATIC-V10912-005` | Regression safety | `PACKAGE VERIFIED / UNITY REQUIRED` | Single camera, no object-scale animation, no table/device motion during the shot, live screen, input lock and first-frame priming remain intact. |
