...
## 2026-06-10 — Post-intro cinematic physical-staging correction V10.9.5

**Classification:** `CURRENT / EARLIER-BLOCKING USER VISUAL CORRECTION`

**New user evidence:** the latest Play Mode frame shows three remaining defects in the post-intro landing: a brief flash/flicker of the final menu composition before the establishing shot begins; the handheld is standing upright instead of lying on the tabletop as it did in the approved earlier composition; and the handheld/table size relationship is not physically believable.

**Authoritative correction contract:**

1. The first visible cinematic frame must already be the exact establishing/start pose. No final-pose frame, ordinary-menu frame, default-camera frame, restore frame or one-frame interpolation mismatch may appear before movement begins.
2. The existing portrait-form handheld must lie flat and remain physically supported by the tabletop for the entire shot. Screen readability is achieved through camera height, pitch and framing—not by standing the device upright or animating its transform.
3. The table and handheld must use believable real-world proportions. As an acceptance baseline, a roughly 14–16 cm tall, 8–10 cm wide, 2–3 cm thick handheld belongs on a roughly 140–180 cm wide, 75–90 cm deep, 72–76 cm high table. Equivalent project-unit ratios are acceptable: the handheld long axis should read at roughly 8–12% of table width and its short axis roughly 9–14% of table depth. Physical scale remains constant; camera travel creates the apparent growth in frame.
4. The complete table, floor, cyclorama, handheld, screen and shadows remain static. Only the existing product camera and its lens/look state may animate.
5. The final frame still matches the ordinary Main Menu camera state exactly and retains a readable front tabletop edge/thickness so the furniture never becomes a floating plane.

**Current truth:** this block records the accepted requirement only. No Runtime correction, Unity compile, TEST EVERYTHING rerun or visual acceptance is claimed yet.

**Exact resume point:** first merge this contract into the maintained ProjectGuide documents. Then repair the authoritative presenter/cinematic owner, update focused automated QA, compile in Unity `6000.0.76f1`, run `Boredom And Dungeons -> TEST EVERYTHING` at `0 blockers / 0 warnings / 0 info`, and review the first frames frame-by-frame plus the complete 24/30/60 FPS shot. Do not continue to later queued work until the flicker, lying rest pose and realistic scale are accepted.
