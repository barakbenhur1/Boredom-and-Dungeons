...
## Current immediate gate — V10.9.5 physical-staging correction

1. Preserve the current repository, Codex-agent system, V10.8.1 tutorial mechanics and V10.9 set/camera work.
2. Record the three user-reported defects in Current, Bugs, the active task, feature/specification, art direction, verification and focused QA before Runtime changes.
3. Eliminate the pre-roll final-pose flash: the first visible rendered frame must already use the exact establishing camera pose and active screen state.
4. Return the existing handheld to a flat, tabletop-supported rest pose. Do not stand, rotate, slide or scale it during the shot.
5. Re-establish realistic table/device dimensions using physical ratios, while preserving the approved device design and complete table construction.
6. Keep the table, device, floor, backdrop and shadows static; animate only the authoritative product camera/lens/look state.
7. Update focused automated QA so upright staging, scale animation and pre-roll/final-pose exposure cannot regress.
8. Compile in Unity `6000.0.76f1`; run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
9. Inspect the first several frames one by one, then review the full shot at 24-equivalent capture, 30 FPS and 60 FPS. Confirm flat contact, believable proportions, no flicker, exact final handoff and no replay on submenu returns.
10. Continue to the queued retro tutorial visual redesign only after user acceptance of this gate.
