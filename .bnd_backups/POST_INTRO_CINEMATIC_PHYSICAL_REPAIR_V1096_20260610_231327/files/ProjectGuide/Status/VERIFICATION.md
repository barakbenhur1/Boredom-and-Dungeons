...
## V10.9.5 user visual evidence and pending verification — 2026-06-10

### Recorded evidence

The user supplied a Play Mode screenshot and direct review after the V10.9 work. The frame shows the handheld standing vertically on the tabletop and reading too large relative to the visible table. The user also reports a small flicker/glimpse of the animation's end/final composition before the animation starts.

### Verification truth

- User visual rejection: **RECORDED**.
- Documentation contract for flicker, flat rest pose and realistic scale: **CAPTURED BY V10.9.5 DOCUMENTATION PACKAGE**.
- Runtime repair: **NOT YET IMPLEMENTED**.
- Focused automated QA update: **NOT YET IMPLEMENTED**.
- Unity compilation after repair: **NOT YET RUN**.
- TEST EVERYTHING after repair: **NOT YET RUN**.
- Frame-by-frame 24/30/60 FPS acceptance: **NOT YET RUN**.
- Final user acceptance: **NOT GRANTED**.

The next implementation must record actual project-unit device/table measurements, prove unchanged object transforms/scales through playback, and provide frame-by-frame evidence that no final/default pose is exposed before frame 0 of the establishing shot.
