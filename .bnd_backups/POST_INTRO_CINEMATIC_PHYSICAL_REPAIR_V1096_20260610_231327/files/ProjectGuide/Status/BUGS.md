...
## Post-intro cinematic V10.9.5 — user-reported visual defects

| ID | Area | Status | Current truth / acceptance condition |
|---|---|---|---|
| `CINEMATIC-V1095-001` | Pre-roll frame continuity | `OPEN / REPRODUCED BY USER` | A brief glimpse/flicker of the final menu/end pose appears before the establishing animation begins. Acceptance requires the first visible frame to be the exact cinematic start pose, followed by continuous motion with no ordinary-menu/default/restore frame and no position, rotation, FOV, exposure or screen-content discontinuity. |
| `CINEMATIC-V1095-002` | Handheld physical rest pose | `OPEN / USER REJECTED` | The handheld currently stands upright. It must lie flat on the tabletop in the approved earlier manner, remain stationary and grounded, and keep a coherent contact shadow. Camera pitch/framing—not object rotation or standing display staging—must preserve screen readability. |
| `CINEMATIC-V1095-003` | Table/device physical scale | `OPEN / USER REJECTED` | The handheld reads far too large relative to the table. Acceptance requires a believable furniture/device ratio with constant object scales; approximate baseline is a 14–16 cm × 8–10 cm handheld on a 140–180 cm × 75–90 cm table, or equivalent ratios of about 8–12% of table width and 9–14% of table depth. The camera alone creates framing enlargement. |

These rows supersede any V10.9 wording that can be read as approving an upright product-display pose or the current proportions. They remain open until code, automated QA, Unity `TEST EVERYTHING`, frame-by-frame review and user visual acceptance all pass.
