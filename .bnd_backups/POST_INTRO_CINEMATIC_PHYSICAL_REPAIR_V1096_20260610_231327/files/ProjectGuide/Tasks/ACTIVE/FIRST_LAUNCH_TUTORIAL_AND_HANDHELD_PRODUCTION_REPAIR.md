...
## V10.9.5 active interruption — flicker, lying rest pose and realistic scale

The latest user Play Mode screenshot/review rejects the current V10.9 staging. Before any further implementation or later task, preserve these requirements as the active blocking correction:

- no final/menu/end-pose flash may appear before the cinematic start; the exact start pose must be applied before the first visible frame;
- the approved existing handheld is a portrait-form object but must physically lie flat on the tabletop, as in the accepted earlier table composition;
- the device remains motionless and supported by the tabletop with a readable contact shadow;
- screen visibility is solved with camera height/pitch/framing, never by standing the device upright;
- table and handheld dimensions must read as real furniture and a real portable device, using the physical ranges/ratios recorded in the maintained 3D specification;
- physical object scale never changes during the shot; apparent growth comes only from camera travel;
- V10.8.1 mechanics, menu behavior, device design, table construction, Codex-agent files and later queue remain protected.

**Implementation state:** documentation captured; Runtime and QA repair not yet implemented.

**Exact resume point:** after this documentation-only package is applied, modify the existing presenter/cinematic owner and focused validator, then run Unity compile, TEST EVERYTHING and the focused frame-by-frame/scale/rest-pose matrix. Do not mark complete from static checks.
