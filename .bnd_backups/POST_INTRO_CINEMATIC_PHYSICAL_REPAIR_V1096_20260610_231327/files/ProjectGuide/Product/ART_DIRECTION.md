...
## Post-intro physical realism correction V10.9.5

The handheld is a small portable object lying flat on a full-size premium wood table. The shot must never read as an oversized upright kiosk, arcade cabinet or giant console on a miniature surface. Preserve the portrait face design while staging its body horizontally against the tabletop with a clear contact shadow.

Use camera height, pitch, distance and restrained lens tuning to reveal the screen. Do not sacrifice believable gravity or proportion to make the UI front-facing. The opening must communicate furniture scale immediately; the final close framing may emphasize the device only because the camera has physically approached it. Object scale, rest orientation, wood grain scale, shadow softness and perspective remain coherent throughout.

A one-frame glimpse of the final/menu composition before the establishing shot is visually unacceptable. The first visible frame must already belong to the intended cinematic composition.
...
Reference board: `ProjectGuide/References/Visual/BOREDOM_AND_DUNGEONS_ART_DIRECTION_REFERENCE_BOARD_V1.jpg`
...
Temporary procedural animation is not final release animation. Detailed coverage, timing ownership, root-motion policy, interruption rules and the production acceptance gate are defined in `ProjectGuide/Features/Animation/PRODUCTION_ANIMATION_REQUIREMENTS_V1.md`.
