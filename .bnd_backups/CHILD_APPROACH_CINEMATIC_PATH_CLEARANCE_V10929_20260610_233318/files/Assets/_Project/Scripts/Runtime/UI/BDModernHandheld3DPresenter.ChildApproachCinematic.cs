...
namespace BoredomAndDungeons
{
    public sealed partial class BDModernHandheld3DPresenter
    {
        ...
        private const float ChildApproachWalkEndsAtSeconds = 3.30f;
        private const float ChildApproachClimbStartsAtSeconds = 3.70f;
        private const float ChildApproachSeatReachSeconds = 5.55f;
        private const float ChildApproachCameraSettleSeconds = 6.55f;
        private const float ChildApproachPowerOnStartsAtSeconds = 6.85f;
        private const float ChildApproachPowerOnEndsAtSeconds = 7.85f;
        ...
        private void ApplyChildApproachCameraPose(float progress)
        {
            ...
            if (seconds <= ChildApproachWalkEndsAtSeconds)
            {
                ...
                // Five restrained child steps. The camera starts behind the
                // chair at the earlier cinematic POV height, then advances with
                // grounded weight transfer instead of a low floating bob.
                float stepEnvelope = Mathf.Sin(Mathf.PI * raw);
                float stepPhase = raw * Mathf.PI * 10f;
                float heelToToe = Mathf.Sin(stepPhase);
                cameraPosition.y +=
                    heelToToe * 0.040f * stepEnvelope;
                cameraPosition.z +=
            ...
            else if (seconds <= ChildApproachSeatReachSeconds)
            {
                ...
                if (climbRaw < 0.46f)
                {
                    float t = SmoothestStep01(climbRaw / 0.46f);
                ...
                cameraPosition.y += effort * 0.045f;
                cameraPosition.x +=
                    Mathf.Sin(climbRaw * Mathf.PI * 2f) * 0.018f;
                rollDegrees =
                    Mathf.Sin(climbRaw * Mathf.PI * 2f) *
                    0.34f * (1f - climbRaw * 0.55f);
            }
