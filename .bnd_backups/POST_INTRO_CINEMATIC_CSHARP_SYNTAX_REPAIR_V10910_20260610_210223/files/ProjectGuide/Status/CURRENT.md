...
## 2026-06-10 — Post-intro cinematic final alignment V10.9.9

**Classification:** `CURRENT / EARLIER-BLOCKING VISUAL ACCEPTANCE GATE`

Fresh Unity evidence after V10.9.8 records `TEST EVERYTHING: PASS` with `0 blockers / 0 warnings / 0 info`. The user visually accepted the cinematic camera movement and no longer reported the giant-scale or tabletop-penetration failures. Two precise visual corrections remain before this task can close:

1. move the physical handheld toward the table's front/lower edge while preserving a deliberate furniture margin;
2. end directly in front of the handheld screen, matching the straight-on relationship of the original pre-cinematic menu rather than the oblique product angle.

V10.9.9 implements those requirements without changing the approved movement timing or the physical scale:

- the handheld root moves from `z = 0` to `z = -3.60`;
- with table front edge `z = -7.20` and device half-length `1.232`, the nearest device edge is `z = -4.832`, leaving approximately `2.368` world units of visible wood rather than touching the edge;
- the final camera is directly above the screen center at `(0, 1.50, -3.19)`, looks vertically down at `(0, -7.17, -3.19)`, and resolves to `Quaternion.Euler(90, 0, 0)`;
- the final lens returns to the established original-menu aspect-aware range `49f -> 36.4f`, now with the camera normal to the screen so it does not create the rejected grazing distortion;
- shadow masks and cinematic light targets follow the new physical device position;
- the obsolete `entryProgress` field and all assignments are removed, eliminating compiler warning `CS0414` without changing behavior.

**Preservation:** the accepted V10.9.8 scale, tabletop clearance, BBH first-frame priming, 4.40-second camera movement, full 3D table, menu controls, tutorial mechanics, textures, models and agent-system files remain intact.

**Exact resume point:** apply V10.9.9, compile in Unity `6000.0.76f1`, run `Boredom And Dungeons -> TEST EVERYTHING` and require `0/0/0`, then visually confirm the front-edge margin and direct final angle. Do not advance to the queued retro tutorial redesign until explicit user acceptance.
