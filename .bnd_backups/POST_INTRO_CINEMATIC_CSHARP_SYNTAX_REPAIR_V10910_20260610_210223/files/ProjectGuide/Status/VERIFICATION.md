...
## V10.9.9 verification truth — 2026-06-10

### Verified incoming baseline

The user-provided `ONE_CLICK_QA_latest.txt` records Unity `6000.0.76f1`, automated `PASS`, `0 blockers`, `0 warnings`, `0 info`, generated UTC `2026-06-10T20:46:19.3839190Z`. The accompanying screenshot and feedback approve the camera movement and narrow the remaining work to device placement and final angle.

A separate Unity compiler warning remains outside the one-click finding count: `CS0414` reports that `entryProgress` is assigned but never used after V10.9.8 removed generic device-entry animation.

### V10.9.9 static/package evidence

- device root moves to `(0,-7.27,-3.60)` while retaining rotation `(90,0,0)` and scale `0.16`;
- final camera and look target share `z = -3.19`, making the view normal to the screen, and final rotation is explicitly `(90,0,0)`;
- the aspect-aware lens is the original menu range `49f -> 36.4f`, used by both settle and resolution changes;
- the spline retains the approved 4.40-second timing but redirects its final alignment to the new direct pose;
- contact shadows and light targets follow the moved device;
- `entryProgress` declaration and assignments are absent;
- both focused validators and all maintained documents protect the new contract.

### Still required

Unity compilation, fresh `TEST EVERYTHING 0/0/0`, visual front-margin/direct-angle review at 24-equivalent/30/60 FPS, internal-menu return tests, and explicit user acceptance.
