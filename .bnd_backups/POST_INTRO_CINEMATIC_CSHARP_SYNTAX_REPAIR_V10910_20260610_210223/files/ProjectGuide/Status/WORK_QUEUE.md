...
## Current immediate gate — V10.9.9 final cinematic alignment

1. Apply V10.9.9 over the installed V10.9.8 working tree without reset, stash or discarded work.
2. Compile in Unity `6000.0.76f1`; require no `CS0414` warning for `entryProgress`.
3. Run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
4. Confirm the handheld is physically closer to the table front/lower edge but retains a clear wood margin; it must not touch or overhang.
5. Confirm the final view is directly in front of the screen plane, with no yaw, roll, trapezoidal skew or grazing perspective.
6. Confirm the accepted 4.40-second movement remains smooth and unchanged in character; inspect the last 12 frames for a clean settle into the exact menu pose.
7. Confirm all three contact-shadow masks and highlights moved with the device and no shadow remains at the old center position.
8. Resize the Game view and return from Settings/Progression/Credits; the direct menu pose and original-menu FOV resolver must remain stable and the cinematic must not replay.
9. Recheck V10.8.1 tutorial regressions and commit only after user visual acceptance.
10. Next task remains `Tasks/QUEUED/FIRST_LAUNCH_TUTORIAL_RETRO_VISUAL_REDESIGN.md`.
