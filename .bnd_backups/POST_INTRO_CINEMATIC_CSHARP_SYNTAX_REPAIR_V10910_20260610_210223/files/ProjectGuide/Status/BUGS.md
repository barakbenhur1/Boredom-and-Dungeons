...
## Post-intro cinematic V10.9.9 — final placement and direct-menu angle implemented

| ID | Area | Status | Current truth / acceptance condition |
|---|---|---|---|
| `CINEMATIC-V1098-001` | Device scale ownership | `RESOLVED / USER VISUALLY ACCEPTED` | V10.9.8 keeps scale `0.16`; the latest user feedback no longer reports an oversized device. |
| `CINEMATIC-V1098-002` | Table penetration | `RESOLVED / USER VISUALLY ACCEPTED` | The latest user feedback lists only placement and final angle; no clipping remains reported. |
| `CINEMATIC-V1099-001` | Front-edge placement | `IMPLEMENTED / VISUAL CONFIRMATION REQUIRED` | Root `z = -3.60` leaves approximately `2.368` units between the nearest device edge and table front edge. Reject both centered/back-heavy placement and edge contact. |
| `CINEMATIC-V1099-002` | Final screen angle | `IMPLEMENTED / VISUAL CONFIRMATION REQUIRED` | Final camera is normal to the horizontal screen: position `(0,1.50,-3.19)`, target `(0,-7.17,-3.19)`, rotation `(90,0,0)`. Reject oblique/grazing or rolled framing. |
| `CINEMATIC-V1099-003` | Compiler hygiene | `IMPLEMENTED / UNITY COMPILE REQUIRED` | Remove unused `entryProgress` field and assignments; warning `CS0414` must not recur. |
| `CINEMATIC-V1099-004` | Shadows and lighting | `IMPLEMENTED / VISUAL CONFIRMATION REQUIRED` | Device contact shadows and light targets follow `DeviceRestPosition.z`; no detached shadow or stale highlight may remain at table center. |

Automated baseline before this patch: Unity `6000.0.76f1`, `TEST EVERYTHING PASS`, `0/0/0`, generated UTC `2026-06-10T20:46:19.3839190Z`.
