...
## V10.9.21 QA contract

Require the dedicated wallpaper partial, supplied wallpaper resource path, exact back-wall object name, final camera/target/FOV, and subtle full-resolution DOF. Confirm the environment calls `BuildExactCinematicBackWallWallpaper()` and does not call `BuildCinematicWallpaperPanels(wallpaperMaterial)` or `BuildCinematicWallSlats(roomWoodMaterial)`.
