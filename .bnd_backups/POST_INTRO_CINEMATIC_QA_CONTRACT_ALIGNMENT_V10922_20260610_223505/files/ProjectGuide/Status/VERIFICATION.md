...
## V10.9.21 verification

The installer validates C# delimiters with a lexical scanner that ignores comments and string literals. It tests first installation, repeat installation, rollback, ZIP integrity, payload hashes and `git diff --check` on a full cinematic fixture.

Unity acceptance remains `PASS | blockers=0, warnings=0, info=0` plus visual approval.
