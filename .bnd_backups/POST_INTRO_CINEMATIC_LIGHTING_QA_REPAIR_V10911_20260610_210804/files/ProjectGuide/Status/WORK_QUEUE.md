...
## Current immediate gate — V10.9.10 compile repair

1. Apply V10.9.10 over the installed V10.9.9 working tree without reset, stash or discarded work.
2. Confirm Unity recompiles with no `CS1525` in either handheld QA file.
3. Run `Boredom And Dungeons -> TEST EVERYTHING`; require `0 blockers / 0 warnings / 0 info`.
4. Confirm the V10.9.9 runtime files are unchanged by this repair.
5. Resume visual confirmation: device near the lower/front table edge with visible margin, and final camera directly facing the device like the original menu.
6. Keep the queued retro tutorial redesign blocked until explicit acceptance of this cinematic.
