...
## V10.9.10 compilation and regression gate

1. `BDModernHandheld3DQA.cs` and `BDFirstLaunchTutorialQA.cs` must not contain a comma followed only by whitespace and `);`.
2. Unity must compile with no `CS1525` or other compiler error.
3. `TEST EVERYTHING` must report `0 blockers / 0 warnings / 0 info`.
4. Runtime files `BDModernHandheld3DPresenter.cs`, `BDModernHandheld3DPresenter.IntroToMainMenuTransition.cs` and `BDModernHandheld3DPresenter.CinematicEnvironment.cs` must remain byte-identical to installed V10.9.9.
5. After compilation, repeat the V10.9.9 visual checks without changing the approved camera movement: front-edge margin, no clipping, direct final view, stable settle and no replay on internal menu returns.
