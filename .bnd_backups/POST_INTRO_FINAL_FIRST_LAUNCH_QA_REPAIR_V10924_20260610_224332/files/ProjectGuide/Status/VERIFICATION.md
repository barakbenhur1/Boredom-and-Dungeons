...
## V10.9.22 verification truth

Incoming evidence reported seven blockers, all caused by stale or inverted camera tokens in QA. The Runtime values are `new Vector3(0f, -1.68f, -4.18f)`, `new Vector3(0f, -7.18f, -4.18f)`, `new Vector3(-0.18f, 0.08f, -6.72f)`, and `Mathf.Lerp(38.8f, 31.8f, fit)`.

The installer performs a direct contract simulation after patching:

- every current physical-staging Require token must exist in the transition Runtime;
- no current Runtime token may remain in the old-camera Forbid block;
- all four current camera tokens must occur exactly once in the first-launch RequiredRuntimeTokens array;
- every retired intermediate camera token must be absent from the required array;
- all protected V10.9.21 implementation files must remain byte-identical.

Fresh Unity `TEST EVERYTHING 0/0/0` is still required.
