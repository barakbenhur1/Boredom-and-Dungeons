...
## Post-intro cinematic V10.9.22 — stale camera QA contracts

| ID | Area | Status | Acceptance condition |
|---|---|---|---|
| `CINEMATIC-V10922-001` | Handheld QA Require block | `FIX IMPLEMENTED / UNITY VERIFICATION REQUIRED` | Physical-staging Require block uses the four current Runtime camera tokens. |
| `CINEMATIC-V10922-002` | Handheld QA Forbid block | `FIX IMPLEMENTED / UNITY VERIFICATION REQUIRED` | Current Runtime values are not forbidden; retired intermediate values remain forbidden. |
| `CINEMATIC-V10922-003` | First-launch aggregate QA | `FIX IMPLEMENTED / UNITY VERIFICATION REQUIRED` | RequiredRuntimeTokens contains the current final camera, look target, approach and FOV exactly once. |
| `CINEMATIC-V10922-004` | Runtime regression safety | `VERIFIED BY INSTALLER` | V10.9.21 Runtime, wallpaper, room, DOF and gameplay files remain byte-identical. |
