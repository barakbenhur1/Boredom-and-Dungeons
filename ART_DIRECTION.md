...
> **Canonical root source.** Every art, UI, VFX, model, material, texture, animation, menu, HUD, icon, typography, and presentation change must read this file. The asset-side document under `Assets/_Project/Design/Visual/` is a synchronized mirror for Unity-side discovery.
