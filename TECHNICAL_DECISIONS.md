...
- `ART_DIRECTION.md` is the canonical root visual source; the asset-side document is a synchronized mirror.
...
- Root `AUDIO_DIRECTION.md` is canonical; the asset-side file is a synchronized mirror.
...
- `Assets/_Project/Design/Runtime/OPEN_BUG_TRACKER.md` is the focused current ledger for defects only.
...
- `PROJECT_STATUS.md` remains the sole owner of project ordering, active stage, overall QA truth, and resume point.
